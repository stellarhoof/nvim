...
local function git_branch(buf)
  local head = fn.FugitiveHead(6, buf.bufnr)
  if #head > 0 then
    return " " .. head
  end
end
...
local function buf_icon(buf)
  local icon = devicons.get_icon(fn.fnamemodify(buf.name, ":e"))
...
local function diagnostics_section(bufnr)
  return function(severity)
    local count =
    #vim.diagnostic.get(bufnr, { severity = vim.diagnostic.severity[fn.toupper(severity)] })
    if count > 0 then
...
return function(current_winid)
  ...
  local buffer = fn.getbufinfo(api.nvim_win_get_buf(vim.g.statusline_winid))[1]
